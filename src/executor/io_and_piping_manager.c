...
static int	redir_manager(int current_input_fd, int current_output_fd);
static int	redirect_fd(int fd, int stdfd);
...
static int	redir_manager(int current_input_fd, int current_output_fd)
{
	if (redirect_fd(current_input_fd, 0) != SUCCESS)
		return (ERROR);
	if (redirect_fd(current_output_fd, 1) != SUCCESS)
		return (ERROR);
	return (SUCCESS);
}
